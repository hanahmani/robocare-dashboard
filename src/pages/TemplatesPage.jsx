  ...
  return (
    <div className="page-shell px-4 sm:px-8 py-6 space-y-6">
      <div className="rounded-3xl border border-brand-100 bg-gradient-to-br from-brand-50 via-white to-medical-50 p-6 shadow-card">
        <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-brand-700">Template studio</p>
        <h1 className="mt-2 text-3xl font-extrabold text-surface-900">Templates</h1>
        <p className="mt-2 max-w-2xl text-sm text-surface-500">
          Browse and manage reusable message templates from a cleaner, branded workspace.
        </p>
      </div>

      <div className="flex items-end justify-between gap-4 mb-4 flex-wrap">
        <div>
          <h2 className="text-lg font-semibold text-surface-900">WhatsApp Meta templates</h2>
          <p className="text-sm text-surface-500">
            Templates loaded from your Meta WhatsApp account. Each card shows the header, body and button components.
          </p>
        </div>
        <div className="text-sm text-surface-500">{metaTemplates.length} templates</div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {metaTemplates.map((template) => {
            ...
            return (
              <Card key={template.id} className="border-brand-100 bg-brand-50/20 overflow-hidden">
                <div className="flex items-start justify-between mb-3 gap-3">
                  <div>
                    <div className="text-[11px] text-surface-400 font-mono">{template.id}</div>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[11px] px-2 py-0.5 bg-surface-100 text-surface-700 rounded-full">
                    {template.language}
                  </span>
                </div>
                ...
                <div className="space-y-3 text-sm">
                  {template.components.map((component, index) => (
                    <div key={`${template.id}-${component.type}-${index}`} className="rounded-2xl border border-surface-200 bg-white p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[11px] font-semibold uppercase tracking-wider text-brand-700">
                          {component.type}
                        </span>
                        {component.format && (
                          <span className="text-[11px] text-surface-400">{component.format}</span>
                        )}
                      </div>
                      {component.text && (
                        <p className="text-xs text-surface-600 whitespace-pre-line">{component.text}</p>
                      )}
                      {component.buttons?.length > 0 && (
                        <div className="mt-2 space-y-2">
                          {component.buttons.map((button, buttonIndex) => (
                            <div key={`${button.text}-${buttonIndex}`} className="rounded-xl border border-dashed border-surface-200 p-2">
                              <div className="text-xs font-medium text-surface-700">{button.text}</div>
                              <div className="text-[11px] text-surface-500 break-all">{button.url}</div>
                            </div>
                          ))}
                        </div>
                      )}
      ...
      <div className="flex items-center justify-between mb-5 gap-3 flex-wrap rounded-3xl border border-surface-200 bg-white/85 p-4 shadow-card backdrop-blur-sm">
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-1.5 text-sm border border-surface-200 rounded-full bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 text-surface-700"
          >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((t) => (
            <Card key={t.id} className="hover:border-surface-300 transition-colors overflow-hidden">
              <div className="flex items-start justify-between mb-3">
                <div className="text-[11px] text-surface-400 font-mono">{t.id}</div>
                <StatusBadge status={t.status} />
              </div>
              <h3 className="font-semibold text-surface-900 mb-1">{t.name}</h3>
              {t.content && (
                <p className="text-xs text-surface-500 line-clamp-2 mt-1">{t.content}</p>
              )}
              {extractVariables(t.content).length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {extractVariables(t.content).map((variable) => (
                    <span key={`${t.id}-${variable}`} className="px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 text-[11px] border border-brand-100">
                      {`{${variable}}`}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-4 pt-3 border-t border-surface-100 flex items-center justify-between">
                <span className="text-xs text-surface-500">Updated {t.updated}</span>
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded hover:bg-alert-50 text-surface-500 hover:text-alert-600"
                    title="Delete"
                  >
      ...
      {showModal && (
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-surface-700 mb-1">Template Name *</label>
              <input
                type="text"
                required
                value={form.name}
            ...
            <div>
              <label className="block text-xs font-medium text-surface-700 mb-1">Content</label>
              <textarea
                value={form.content}
              ...
              />
              <div className="mt-2 rounded-2xl border border-dashed border-surface-200 bg-surface-50 p-3 space-y-2">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="text-xs font-medium text-surface-700">Variable hints</div>
                  <div className="text-[11px] text-surface-500">Use braces like {'{name}'} and {'{date}'}</div>
                </div>
                <div className="flex flex-wrap gap-2">
                  {selectedTemplateVariables.length > 0 ? selectedTemplateVariables.map((variable) => (
                    <span key={variable} className="px-2 py-0.5 rounded-full bg-white border border-surface-200 text-[11px] text-surface-700">
                      {`{${variable}}`}
                    </span>
                  )) : (
                    <span className="text-xs text-surface-500">No variables detected yet.</span>
                  )}
                </div>
                <div className="rounded-2xl border border-surface-200 bg-white p-3 text-xs text-surface-600 whitespace-pre-line">
                  <div className="font-medium text-surface-700 mb-1">Live preview</div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                className="px-4 py-1.5 text-sm border border-surface-200 rounded-full hover:bg-surface-50 text-surface-700"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-4 py-1.5 text-sm bg-brand-600 text-white rounded-full hover:bg-brand-700 shadow-glow"
              >
